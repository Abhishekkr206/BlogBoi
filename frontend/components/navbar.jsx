...
export function Sidebar({ isExpanded, setIsExpanded }) {
  ...
  return (
    <div
      ...
      } bg-white border-r border-gray transition-all duration-300 flex flex-col h-screen fixed z-40`}
    >
      {/* Menu Button */}
  ...
  return (
    <nav className="w-full bg-white border-b border-gray shadow-sm sticky top-0 z-30">
      <div className="px-6 flex justify-between items-center py-2">
        <Link to="/">
          <h1 className="text-4xl text-gray-900 font-oswald cursor-pointer">BlogBo!</h1>
        </Link>
          ...
          {user ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium bg-black text-white hover:bg-gray-800 rounded-md transition"
              >
                Sign Up
              </button>
            </div>
