...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      socket.off("receive_message", handleActiveChatReceive);
    };
  }, [receiverId]);
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-gray-50">
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-3 h-full w-full">
                  ...
                  {isMe && (
                    <CheckCheck
                      className={`w-4 h-4 mt-1 self-end transition-colors ${
                        msg.isRead ? "text-white" : "text-blue-400"
                      }`}
                    />
                  )}
      ...
      <form onSubmit={handleSend} className="p-4 bg-white border-t flex gap-2 shrink-0">
        <input
          className="flex-1 border border-gray-300 rounded-full px-5 py-2 outline-none focus:ring-1 focus:ring-green-500"
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="Type a message..."
        />
        <button type="submit" className="bg-gray-600 text-white px-8 py-2 rounded-full hover:bg-blue-700">
          Send
        </button>
      </form>
    </div>
